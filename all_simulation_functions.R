...
# OLDER DETECTION FUNCTION LEFT FOR REFERENCE WHEN TEST SURVEILLANCE IS IMPLEMENTED AGAIN
...
f_detection <-function(vertex_variables,surveillance_schedule_t){  
  ...
  if(nrow(test_infected_vertex_test_1)>0){
    ...
    # tests_parameters$share_tested[1]
  ...
  if(nrow(test_infected_vertex_test_3)>0){
    
    output_detection_3 <-  data.frame("ID_vertex" = as.character(),
                                      "test_type" = as.numeric())
    
    
    test_infected_vertex_test_3$pigs_dead <- test_infected_vertex_test_3[,c("carcass")]
    test_infected_vertex_test_3$total_pigs  <- rowSums(test_infected_vertex_test_3[,c(compartment_list[1:compartment_num-1])])
    test_infected_vertex_test_3$detections  <- 0
    test_infected_vertex_test_3$detections[test_infected_vertex_test_3$pigs_dead >=  tests_parameters$other_parameter_1[3] |
                                             test_infected_vertex_test_3$pigs_morbid/test_infected_vertex_test_3$total_pigs >  tests_parameters$other_parameter_2[3] ] <- 3
    
    
    if(nrow(test_infected_vertex_test_3)>0){ # draw the tests only if there are any infected animals selected for testing
      output_detection_3 <- test_infected_vertex_test_3[test_infected_vertex_test_3$detections > 0, c("ID_vertex", "detections")]
      output_detection  <- rbind(output_detection,output_detection_2)
    }
  }
