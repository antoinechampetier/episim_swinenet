...
mk_surveillance<- function(vertex_key, surveillance_parameter, surveillance_policy){
  ...
  targetted_farms = vertex_parameters$ID_vertex[ !is.na(vertex_parameters$farm_type_ML) ]
  number_farms_sampled = nrow(vertex_parameters[vertex_parameters$farm_type_ML == "Ring_ins" & !is.na(vertex_parameters$farm_type_ML) ,])
  targetted_farms_id = sample(length(targetted_farms), number_farms_sampled, replace = FALSE)
  #targetted_farms = targetted_farms[targetted_farms_id,]
  targetted_farms = targetted_farms_id
